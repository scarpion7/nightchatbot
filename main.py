...
* **Random Voice Messages:**
    ...
    * The `gender_handler` now selects a random phrase and asks the user to send it as a voice message, transitioning to a new `Form.AWAITING_VOICE_MESSAGE` state.
    * A new handler `voice_message_handler` (for `F.voice` messages in `Form.AWAITING_VOICE_MESSAGE` state) captures the user's voice message, stores its `file_id` and the expected phrase, and then moves the user to the `Form.VILOYAT` state.
    * An `invalid_voice_message_handler` is added to guide users who send non-voice messages in the `AWAITING_VOICE_MESSAGE` state.

* **Proceed to next stage after voice message:** The bot now explicitly waits for a voice message in the `AWAITING_VOICE_MESSAGE` state before proceeding to the `VILOYAT` selection.

* **Include voice message in posts:**
    * The `send_application_to_destinations` function has been updated to retrieve the `voice_message_file_id` and `voice_message_text` from the state data.
    * When sending the application summary to admin chats/channels, it now includes the text "Ovozli tasdiqlash: [original phrase]" and then sends the actual voice message as a separate reply to the text message.

* **Hyperlink in applications:**
    * In the `send_application_to_destinations` function, the "profilga havola" text has been explicitly added next to the user's profile link (`tg://user?id={user.id}`) in both the `admin_message_text` and `channel_text` for clarity and direct access to the user's profile.

* **Aiogram 3.20.0 compliance:** All new additions and modifications are made using Aiogram 3.x syntax, including `StatesGroup`, `F` filters, and `InlineKeyboardBuilder`.
